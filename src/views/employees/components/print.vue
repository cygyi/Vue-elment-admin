...
<script>
import { getTransferPosition, getPersonalDetail } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      const { data: userInfo } = await getStaffPhoto(this.userId) // 获取个人基本信息(顶部)
      const { data: detail } = await getPersonalDetail(this.userId) // 获取个人基本信息(底部)
      this.formData = { ...detail, ...userInfo }
    },
    async getJobDetail() {
      const { data: userInfo } = await getStaffPhoto(this.userId)
      const { data: jobInfo } = await getTransferPosition(this.userId) // 获取个人基本信息
